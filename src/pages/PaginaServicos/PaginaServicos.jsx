...
import { ContainerFather, ContainerInput, Cards, BodyCards, JobCard, Data, Botoes, Button, CssTextField } from "./style"
...
export default class PaginaServicos extends React.Component {
    ...
    getAllJobs = () => {
        axios
            ...
            .then((response) => {
                this.setState({ jobs: response.data.jobs, filterJobs: response.data.jobs })
    ...
    render() {
            ...
            return (
                ...
                <BodyCards key={job.id}>
                    <JobCard key={job.id} job={job}>
                        <h3>{job.title}</h3>
                        <Data><b>Disponível até:</b><br /><br /><b>{convertDate(job.dueDate)}</b></Data>
                        <p><b>Valor: </b><b>R${job.price.toFixed(2)}</b></p>
                        <Botoes>
                            <Button><b><GrAdd /> Ver detalhes</b></Button>
                            <Button><b>Add ao Carrinho <MdOutlineAddShoppingCart /></b></Button>
                        </Botoes>
                    </JobCard>
                </BodyCards>
            )
        ...
        return (
            <ContainerFather>
                <ContainerInput>
                    <CssTextField id="outlined-basic" label="Busca por título" variant="outlined" value={this.state.search} onChange={this.listSearch} />
                    <CssTextField id="outlined-basic" label="Valor máximo" variant="outlined" value={this.state.maxVal} onChange={this.listMaxVal} />
                    <CssTextField id="outlined-basic" label="Valor mínimo" variant="outlined" value={this.state.minVal} onChange={this.listMinVal} />
                    <FormControl sx={{ m: 0, minWidth: 250 }}>
                        <InputLabel id="demo-simple-select-label">Ordenar</InputLabel>
                        <Select labelId="demo-simple-select-label" id="demo-simple-select" label="Ordenar" value={this.state.order} onChange={this.listOrder}>
